...
选项：
  ...
  --dir <PATH>                        用于扫描测试文件的基础目录
  --ui                                启用 UI
  --open                              自动打开 UI（默认：!process.env.CI）
  --api [PORT]                        指定服务器端口；如果为 true，则默认为 51204
  --silent [VALUE]                    静默测试的控制台输出。使用 'passed-only' 可仅查看失败测试的日志
  ...
  --pool <POOL>                       指定不在浏览器中运行时使用的进程池（默认：forks）
  --execArgv <OPTION>                 生成工作线程或子进程时向 Node.js 传递其他参数
  ...
  --slowTestThreshold <THRESHOLD>     将测试或测试套件视为运行缓慢的阈值（默认：<duration>）
  --teardownTimeout <TIMEOUT>         默认 teardown 超时时间（以毫秒为单位）（默认：10000）
  --cache                             启用缓存
  ...
  --clearScreen                       在监视模式重新运行测试时清除终端（默认：true）
...
选项：
  ...
  --shims                       启用 cjs 和 esm 兼容垫片（默认：false）
  ...
  --env-prefix <PREFIX>         注入到打包内容中的环境变量前缀（默认：VITE_PACK_,TSDOWN_）
