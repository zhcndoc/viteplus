...
基本配置
    -c, --config=<./.oxlintrc.json>  Oxlint 配置文件
                              ...
                              * JavaScript/TypeScript 配置文件属于实验性功能，要求
                              通过 Node.js 运行
                              * 你可以在配置文件中使用注释。
                              * 尝试兼容 ESLint v8 的格式
        --tsconfig=<./tsconfig.json>  覆盖用于导入解析的 TypeScript 配置。
                              Oxlint 会自动为每个文件查找相关的 `tsconfig.json`。
                              仅当项目使用非标准的 tsconfig 名称或位置时才使用此选项。
...
允许/禁止多个 lint
   ...
   * `pedantic`    - 比较严格或偶尔会产生误报的 lint
   * `perf`        - 可以用更高性能的方式编写的代码
   * `style`       - 应以更符合惯用方式编写的代码
   * `restriction` - 禁止使用语言和库特性的 lint
   * `nursery`     - 仍在开发中的新 lint
   * `all`         - 除 `nursery` 外的上述所有类别。不自动启用插件
...
修复问题
        ...
        --fix-dangerously     应用具有风险的修复和建议

忽略文件
        --ignore-path=PATH    指定用作 `.eslintignore` 的文件
        --ignore-pattern=PAT  指定要忽略的文件模式（在 `.eslintignore` 中的设置之外）
...
处理警告
        ...
        --max-warnings=INT    指定警告阈值。如果项目中的警告级规则违规过多，
                              可使用该选项强制以错误状态退出
...
输出
    -f, --format=ARG          使用特定的输出格式。可能的值：`checkstyle`、
                              ...
                              `sarif`、`stylish`、`unix`
        --debug=OPTIONS       启用调试输出选项。选项以逗号分隔。可能的
                              值：
                               * `files` - 输出将进行 lint 的文件列表，然后退出。
                               * `timings` - 启用按规则统计的耗时信息。

其他
        --silent              不显示任何诊断信息
        --no-error-on-unmatched-pattern  未选择文件进行 lint 时不要以错误退出
                              （例如，在应用忽略模式后）
        --threads=INT         要使用的线程数。设置为 1 时仅使用 1 个 CPU 核心。
        --print-config        输出将使用的配置。存在此选项时，不执行 lint，
                              且仅配置相关选项有效。
