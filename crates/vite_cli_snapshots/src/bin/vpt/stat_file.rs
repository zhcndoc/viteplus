...
/// `test -d` assertions keep their predicate fidelity in snapshots. The
/// assert flags (states: `file`, `dir`, `missing`) additionally fail the
/// step on mismatch, preserving shell `test` guard semantics under the
/// runner's line-boundary failure flow.
pub fn run(args: &[String]) -> Result<(), Box<dyn std::error::Error>> {
    ...
    while i < args.len() {
        match args[i].as_str() {
            flag @ ("--assert" | "--assert-not") => {
                ...
                if !matches!(state, "file" | "dir" | "missing") {
                    return Err(
                        format!("unknown state `{state}`; expected file, dir, or missing").into()
                    );
                }
    ...
    for file in paths {
        let state = match std::fs::metadata(file) {
