...
选项：
  ...
  --concurrency-limit <N>  同时运行的最大任务数（默认：4）
  --parallel               不按依赖顺序运行任务；除非指定 `--concurrency-limit`，否则并发数不受限制
...
选项：
  ...
  --cache                  强制为所有任务和脚本启用缓存
  --no-cache               强制为所有任务和脚本禁用缓存
  --log <MODE>             设置输出模式：交错（默认）、标记或分组
  --concurrency-limit <N>  可同时运行的最大任务数（默认：4）
